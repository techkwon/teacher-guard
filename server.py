...
DANGER_KW = [
    # 살해 — 타동 위협 활용형 + 2인칭 + bare 명령·관용어(자동사 과장 '배고파 죽겠다'는 미포함)
    "죽이겠", "죽일거", "죽일것", "죽일게", "죽여버리", "죽여 버리", "죽여버려", "죽여 버려",
    "죽여버린", "죽여버릴", "죽여놓", "죽여라", "죽여놔", "다죽여", "다 죽여", "패죽", "패 죽", "쳐죽", "쳐 죽",
    "때려죽", "두들겨패", "두들겨 패", "죽창들", "죽창으로", "박살내겠", "박살낼", "끝장내겠", "끝장낼",
    "피를보겠", "피를 보겠", "씨를말", "반죽음", "초주검",
    "너죽", "너 죽", "넌죽", "넌 죽", "죽을줄알", "죽을 줄 알",  # 2인칭(거리 두면 한계)
    "없애버리", "없애 버리", "없애버려", "없애버린", "없애버릴",
    "산채로묻", "산 채로 묻", "파묻어버", "생매장",
    "쏴버리", "쏴 버리", "쏴죽", "쏴 죽", "목따", "목 따", "목을따", "목을 따", "목을딴",
    # 흉기 — 사용·소지 맥락(단독 '칼/흉기' 금지: 칼국수/회칼/식칼/칼춤/흉기 대피훈련 회피)
    "흉기로", "흉기휘두", "흉기들고", "흉기 들고", "칼부림", "칼로찌", "칼로 찌", "칼로찔", "칼로 찔",
    "칼로베", "칼로 베", "칼로쑤", "칼로 쑤", "칼들고", "칼 들고", "칼가지고", "칼 가지고", "칼을들", "칼을 들",
    "칼빼들", "칼 빼들", "칼휘두", "칼 휘두", "칼들이대", "칼 들이대", "식칼들", "회칼들",
    "찌르겠", "찔러죽", "찔러 죽", "찔러버리", "찔러 버리", "찔러버려", "찔러버린", "찔러버릴", "찔렀어",
    "베어버리", "베어 버리", "베어버려", "베어버린", "낫으로", "낫들고", "도끼로", "도끼들",
    "몽둥이", "쇠파이프", "각목으로", "각목들", "망치로때", "망치로 때", "망치로내려", "망치로 내려",
    # 총기·약품 — 사용·소지 맥락(단독 '총/권총/염산/황산/휘발유' 금지: 물총/총무/과학실험 회피)
    "엽총으로", "엽총들", "권총으로", "권총들", "권총 들", "권총가지", "총들고", "총 들고", "총겨누", "총 겨누",
    "총가지고", "총 가지고", "총으로쏴", "총으로 쏴", "총쏘", "사제총", "염산뿌", "염산 뿌", "염산을뿌", "염산테러",
    "황산뿌", "황산 뿌", "황산테러", "휘발유뿌", "휘발유 뿌", "휘발유붓", "휘발유끼얹",
    # 가해·폭행 — 위협 활용형(단독 '해치/부러뜨리/휘두르' 금지)
    "해치겠", "해치려", "해친다", "해칠거", "해칠것", "해코지",
    "뼈를부러", "뼈를 부러", "다리를부러", "다리를 부러", "팔을부러", "팔을 부러", "부러뜨리겠", "부러뜨릴", "부러뜨려버",
    "분질러버", "팔다리분질", "밀어버리겠", "밀어버린", "밀어버릴", "패버리겠", "때려버리겠", "주먹휘두", "주먹 휘두",
    # 침입·습격
    "찾아오겠", "찾아가겠", "쳐들어",
    # 방화 — 위협 활용형(단독 '방화/태워/폭파' 금지: 방화벽/방화문/고기 태움/폭파해체 다큐 회피)
    "불지르", "불지를", "불지른", "불질러", "불질렀", "불을지르", "불을지를", "불을지른", "불을질러",
    "불태워", "불 태워", "불싸지", "불싸질러", "불놓겠", "불 놓겠", "태워버리겠", "태워버린", "태워버릴",
    "방화하", "방화범", "방화저지", "기름붓고불", "기름 붓고 불",
    # 폭발 — 위협 결합형
    "폭파하", "폭파시키", "폭파해버", "폭발물", "폭발시키", "터뜨려버", "터트려버", "터뜨리겠", "터트리겠", "사제폭탄",
    # 중대 협박조
    "가만안두", "가만 안 두", "가만안둔", "가만 안 둔", "가만안둬", "가만 안 둬", "가만두지않", "가만두지 않",
    "손봐주겠", "손 봐주겠", "혼쭐내겠", "혼쭐 내겠", "혼쭐낼", "혼쭐을내", "혼쭐을 내",
    # 자·타해 위기 — 의지형으로 한정('자해하지 않게 상담' 등 부정·지도 맥락 회피)
    "자해하겠", "자해하려", "자해할", "자살하겠", "자살하려", "자살할", "죽어버리겠", "죽어 버리겠",
]
...
def get_legal_basis(query: str = "", type_id: str = "", law_id: str = "") -> str:
    ...
    if law_id and law_id in LEGAL_BY_ID:
        ...
        for r in LEGAL["records"]:
            ...
            if n:
                # 매뉴얼(권고)·미확인은 -1 감점 → '법명+조문' 검색에서 법률 본문이 항상 위로.
                # (매뉴얼이 법조문을 인용해 토큰이 함께 매칭돼도 법률을 우선; 매뉴얼은 [⚠️] 태그로 구분)
                weak = 1 if r.get("authority_level") in ("매뉴얼(권고)", "미확인") else 0
                scored.append((n - weak, r))
        # 보정 점수 ↓, 동점 시 권위(법률>시행령>고시>매뉴얼) 우선
